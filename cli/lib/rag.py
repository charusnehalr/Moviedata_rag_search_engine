...
def question_answer_summarize(question, limit=5):
   movies = load_movies()
   hs = HybridSearch(movies)
   rrf_results = hs.rrf_search(question, k= 60, limit=5)
   print("Search Results")
   for res in rrf_results:
    print(f"- {res['title']}")
   rag_summary_results = question_summary(question, rrf_results)
   print("Answer:")
   print(rag_summary_results)
